...
{
    public class PurBillController : BasePurApiController
    {
        ...
        {
            var chk = await permission.HasPermission(281, PermissionType.Show);
            if (!chk)
            {
            ...
            try
            {
                var items = await purServiceManager.PurTransactionService.GetAll(x => x.IsDeleted == false && (x.TransTypeId == 1 || x.TransTypeId == 7 || x.TransTypeId == 10));
                if (items.Succeeded)
                {
        ...
        [HttpPost("Search")]
        public async Task<IActionResult> Search(PurTransactionBillFilterDto filter)
        {
            var chk = await permission.HasPermission(281, PermissionType.Show);
            if (!chk)
            {
            ...
            try
            {
                ...
                var items = await purServiceManager.PurTransactionService.GetAllVW(x =>
                    x.IsDeleted == false &&
                    (x.TransTypeId == 1 || x.TransTypeId == 7 || x.TransTypeId == 10) &&
                    ...
                    (string.IsNullOrEmpty(filter.Code) || (x.Code != null && x.Code.CompareTo(filter.Code) >= 0 && x.Code.CompareTo(filter.CodeTo) <= 0)) &&
                    ...
                    (filter.BillType == 0 || x.BillType == filter.BillType) &&
            ...
            {
                return Ok(await Result<PurTransactionBillFilterDto>.FailAsync($"======= Exp in Search MESSAGE: {ex.Message}"));
            }
        }
        ...
        //[HttpPost("Add")]
        //public async Task<IActionResult> Add(PurTransactionBillDto obj)
        //{
        //    try
        //    {
        //        var chk = await permission.HasPermission(281, PermissionType.Add);
        //        if (!chk)
        //            return Ok(await Result.AccessDenied("AccessDenied"));
        ...
        //            }
        //        }
        //        var add = await purServiceManager.PurTransactionService.AddPurchaseBill(obj);
        //        return Ok(add);
        //    }
        //    catch (Exception ex)
        //    {
