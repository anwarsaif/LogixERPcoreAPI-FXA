...
{
    public class AccOpeningBalanceApiController : BaseAccApiController
    {
        ...
        public AccOpeningBalanceApiController(
        ...
        {
            var chk = await permission.HasPermission(68, PermissionType.Show);
            if (!chk)
            {
            ...
            try
            {
                var items = await accServiceManager.AccJournalMasterService.GetAllVW(x => x.FlagDelete == false && x.FacilityId == _session.FacilityId && x.DocTypeId == 4 && x.FinYear == _session.FinYear);
                if (items.Succeeded)
                {
    ...
    && (filter.BranchId == 0 || (x.BranchId == filter.BranchId))
       && ((filter.BranchId != 0) || branchsId.Contains(x.BranchId.ToString()))
        ...
        [HttpPost("Add")]
        public async Task<IActionResult> Add(OpeningBalanceDtoVW obj)
        {
            var chk = await permission.HasPermission(68, PermissionType.Add);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
            try
            {
                ...
                {
                    return Ok(await Result<OpeningBalanceDtoVW>.FailAsync(localization.GetMessagesResource("dataRequire")));
                }

                var add = await accServiceManager.AccJournalMasterService.AddOpeningBalance(obj);
                return Ok(add);

            }
            catch (Exception ex)
            {
                return Ok(await Result<OpeningBalanceDtoVW>.FailAsync($"======= Exp in Acc Opening Balance : {ex.Message}"));
            }
        }
        ...
        [HttpPost("Edit")]
        public async Task<IActionResult> Edit(OpeningBalanceEditDtoVW obj)
        {
            var chk = await permission.HasPermission(68, PermissionType.Edit);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
            try
            {
                if (!ModelState.IsValid)
                {
                    return Ok(await Result<OpeningBalanceEditDtoVW>.FailAsync(localization.GetMessagesResource("dataRequire")));
                }
                var Edit = await accServiceManager.AccJournalMasterService.UpdateOpeningBalance(obj);
                return Ok(Edit);
            }
            catch (Exception ex)
            {
                return Ok(await Result<OpeningBalanceEditDtoVW>.FailAsync($"======= Exp in Acc Opening Balance  edit: {ex.Message}"));
            }
        }
        ...
        {
            var chk = await permission.HasPermission(68, PermissionType.Delete);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
