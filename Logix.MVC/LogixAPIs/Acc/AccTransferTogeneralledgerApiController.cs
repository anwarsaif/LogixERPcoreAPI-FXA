...
namespace Logix.MVC.LogixAPIs.Acc
{
    ...
    public class AccTransferTogeneralledgerController : BaseAccApiController
    {
        ...
        public AccTransferTogeneralledgerController(
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            try
            {
                ...
                var items = await accServiceManager.AccJournalMasterService.GetAllVW(x => x.FlagDelete == false && x.FacilityId == _session.FacilityId && x.FinYear == _session.FinYear &&x.StatusId==1 && Posting_By_User_Doc_Type.Equals("1") || Acc_Posting.Contains(x.DocTypeId.ToString()));
                if (items.Succeeded)
                {
    ...
    x.FacilityId == _session.FacilityId && x.FlagDelete == false && x.StatusId == 1 && (Posting_By_User_Doc_Type == "1" || Acc_Posting.Contains(x.DocTypeId.ToString()))
    ...
    && (string.IsNullOrEmpty(filter.JCode) || (x.JCode != null && x.JCode.CompareTo(filter.JCode) >= 0 && x.JCode.CompareTo(filter.JCode2) <= 0))
     ...
     && (string.IsNullOrEmpty(filter.ReferenceCode) || (x.ReferenceCode != null && x.ReferenceCode.Contains(filter.ReferenceCode)))
                ...
                if (items.Succeeded)
                {
                    ...
                    return Ok(await Result<List<JournalMasterVM2>>.SuccessAsync(result, ""));
                }
        ...
        [HttpPost("UpdateTransferGeneralTo")]
        public async Task<IActionResult> UpdateTransferGeneralTo(AccJournalMasterStatusDto obj)
        {
            var chk = await permission.HasPermission(70, PermissionType.Edit);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
            try
            {
                ...
                obj.StatusId = 2;
                var add = await accServiceManager.AccJournalMasterService.UpdateTransferGeneralTo(obj);
        ...
        [HttpPost("UpdateTransferGeneralAllTo")]
        public async Task<IActionResult> UpdateTransferGeneralAllTo(AccJournalMasterStatusDto obj)
        {
            var chk = await permission.HasPermission(70, PermissionType.Edit);
            if (!chk)
            {
            ...
            try
            {
                ...
                obj.StatusId = 2;
                var add = await accServiceManager.AccJournalMasterService.UpdateTransferGeneralAllTo(obj);
                return Ok(add);
        ...
        [HttpPost("UpdateACCJournalComment")]
        public async Task<IActionResult> UpdateACCJournalComment(AccJournalMasterStatusDto obj)
        {
            var chk = await permission.HasPermission(70, PermissionType.Edit);
            if (!chk)
            {
            ...
            try
            {
                ...
                obj.StatusId = 3;
                var add = await accServiceManager.AccJournalMasterService.UpdateACCJournalComment(obj);
