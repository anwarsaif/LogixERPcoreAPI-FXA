...
{
    public class AccJournalApiController : BaseAccApiController
    {
        ...
        public AccJournalApiController(
        ...
        {
            var chk = await permission.HasPermission(69, PermissionType.Show);
            if (!chk)
            {
            ...
            try
            {
                var items = await accServiceManager.AccJournalMasterService.GetAllVW(x => x.FlagDelete == false && x.FacilityId == _session.FacilityId && x.DocTypeId == 3 && x.FinYear == _session.FinYear);
                if (items.Succeeded)
                {
        ...
        public async Task<IActionResult> Search(AccJournalMasterfilterDto filter)
        {
            ...
            try
            {
                ...
                //                }
                var items = await accServiceManager.AccJournalMasterService.Search(filter);
        ...
        {
            var chk = await permission.HasPermission(69, PermissionType.Add);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
            try
            {
                ...
                obj.AccJournalMasterDto.DocTypeId = 3;
        ...
        {
            var chk = await permission.HasPermission(69, PermissionType.Edit);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
        ...
        {
            var chk = await permission.HasPermission(69, PermissionType.Delete);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
        ...
        #region "transactions Review"
        [HttpPost("SearchReview")]
        public async Task<IActionResult> SearchReview(AccJournalMasterfilterDto filter)
        {
            var chk = await permission.HasPermission(1564, PermissionType.Show);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
