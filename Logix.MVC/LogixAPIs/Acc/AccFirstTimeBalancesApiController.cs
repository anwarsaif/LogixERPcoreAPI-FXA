...
{
    public class AccFirstTimeBalancesApiController : BaseAccApiController
    {
        ...
        public AccFirstTimeBalancesApiController(
        ...
        {
            var chk = await permission.HasPermission(530, PermissionType.Show);
            if (!chk)
            {
            ...
            try
            {
                var items = await accServiceManager.AccJournalMasterService.GetAllVW(x => x.FlagDelete == false && x.FacilityId == _session.FacilityId && x.DocTypeId == 27 && x.FinYear == _session.FinYear);
                if (items.Succeeded)
                {
    ...
    && (filter.BranchId == 0 || (x.BranchId == filter.BranchId))
       && ((filter.BranchId != 0) || branchsId.Contains(x.BranchId.ToString())) 
        ...
        [HttpPost("Add")]
        public async Task<IActionResult> Add(FirstTimeBalanceDtoVW obj)
        {
            var chk = await permission.HasPermission(530, PermissionType.Add);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
            try
            {
                ...
                {
                    return Ok(await Result<FirstTimeBalanceDtoVW>.FailAsync(localization.GetMessagesResource("dataRequire")));
                }

                var add = await accServiceManager.AccJournalMasterService.AddFirstTimeBalances(obj);
                return Ok(add);

            }
            catch (Exception ex)
            {
                return Ok(await Result<FirstTimeBalanceDtoVW>.FailAsync($"======= Exp in Acc First Time Balance : {ex.Message}"));
            }
        }
        ...
        [HttpPost("Edit")]
        public async Task<IActionResult> Edit(FirstTimeBalanceEditDtoVW obj)
        {
            var chk = await permission.HasPermission(530, PermissionType.Edit);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
            try
            {
                if (!ModelState.IsValid)
                {
                    return Ok(await Result<FirstTimeBalanceEditDtoVW>.FailAsync(localization.GetMessagesResource("dataRequire")));
                }
                var Edit = await accServiceManager.AccJournalMasterService.UpdateFirstTimeBalance(obj);
                return Ok(Edit);
            }
            catch (Exception ex)
            {
                return Ok(await Result<FirstTimeBalanceEditDtoVW>.FailAsync($"======= Exp in Acc First Time Balance  edit: {ex.Message}"));
            }
        }
        ...
        {
            var chk = await permission.HasPermission(530, PermissionType.Delete);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
