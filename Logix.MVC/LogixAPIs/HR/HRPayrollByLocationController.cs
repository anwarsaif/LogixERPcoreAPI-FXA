...
namespace Logix.MVC.LogixAPIs.HR
{
    ...
    //   تقرير توزيع الرواتب حسب الموقع
    public class HRPayrollByLocationController : BaseHrApiController
    {
        ...
        public HRPayrollByLocationController(IHrServiceManager hrServiceManager, IPermissionHelper permission, ICurrentData session, ILocalizationService localization)
        {
        ...
        {
            var chk = await permission.HasPermission(1012, PermissionType.Show);
            if (!chk)
            {
            ...
            try
            {
                ...
                //    }
                //}
                //var items = await hrServiceManager.HrPayrollDService.GetPayrollReports(filter,1);
                //if (items.Succeeded)
                //{
                ...
                var items = await hrServiceManager.HrPayrollDService.PayrollByLocationSearch(filter);
                return Ok(items);
