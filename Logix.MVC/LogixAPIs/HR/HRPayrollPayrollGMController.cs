...
namespace Logix.MVC.LogixAPIs.HR
{
    ...
    //  إعتماد مسير الرواتب من قبل المدير العام
    public class HRPayrollGMController : BaseHrApiController
    {
        ...
        public HRPayrollGMController(IHrServiceManager hrServiceManager, IPermissionHelper permission, ICurrentData session, ILocalizationService localization, IMainServiceManager mainServiceManager)
        {
        ...
        {
            var chk = await permission.HasPermission(298, PermissionType.Show);
            if (!chk)
            {
            ...
            try
            {
               ...
               (filter.FinancelYear == 0 || filter.FinancelYear == null || filter.FinancelYear == e.FinancelYear)
                );
                if (items.Succeeded)
                {
                    if (items.Data.Count() > 0)
                    {
                        return Ok(await Result<List<HrPayrollVw>>.SuccessAsync(items.Data.ToList(), ""));
                    }
        ...
        public async Task<IActionResult> Edit(long Id)
        {
            try
            {
                var chk = await permission.HasPermission(298, PermissionType.Edit);
                if (!chk)
                    return Ok(await Result.AccessDenied("AccessDenied"));
        ...
        public async Task<IActionResult> AApprove(ApproveRejectPayrollDto obj)
        {
            try
            {
                ...
                var item = await hrServiceManager.HrPayrollService.ChangeStatusPayroll(obj, 4);
                return Ok(item);
        ...
        // إعتماد عند التحديد
        [HttpGet("AApprove2")]
        public async Task<IActionResult> AApprove2(string msIds)
        {
            try
            {
                var chk = await permission.HasPermission(298, PermissionType.Edit);
                ...
                var item = await hrServiceManager.HrPayrollService.ChangeStatusPayroll(msIds, 4);
                return Ok(item);
        ...
        public async Task<IActionResult> Reject(ApproveRejectPayrollDto obj)
        {
            try
            {
                ...
                var item = await hrServiceManager.HrPayrollService.ChangeStatusPayroll(obj, 7);
                return Ok(item);
        ...
        // رفض عند التحديد
        [HttpGet("Reject2")]
        public async Task<IActionResult> Reject2(string msIds)
        {
            try
            {
                var chk = await permission.HasPermission(298, PermissionType.Edit);
                ...
                var item = await hrServiceManager.HrPayrollService.ChangeStatusPayroll(msIds, 7);
                return Ok(item);
