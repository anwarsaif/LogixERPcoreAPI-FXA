...
namespace Logix.MVC.LogixAPIs.HR
{
    ...
    //  مخصص الاجازات
    public class HRProvisionsVacationController : BaseHrApiController
    {
        ...
        public HRProvisionsVacationController(IHrServiceManager hrServiceManager, IPermissionHelper permission, ICurrentData session, ILocalizationService localization, IMainServiceManager mainServiceManager, IMapper mapper, IAccServiceManager accServiceManager)
        {
    ...
    public async Task<IActionResult> GetPagination([FromBody] HrProvisionFilterDto filter, int take = Pagination.take, int? lastSeenId = null)
    {
      try
      {
        var chk = await permission.HasPermission(1519, PermissionType.Show);
        if (!chk)
        {
    ...
    [HttpPost("Search")]
        ...
        {
            var chk = await permission.HasPermission(1519, PermissionType.Delete);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
        ...
        {
            var chk = await permission.HasPermission(1519, PermissionType.Edit);
            if (!chk)
            {
            ...
            try
            {
                ...
                var getJournal = await accServiceManager.AccJournalMasterService.GetOne(j => j.ReferenceNo == Id && j.DocTypeId == 54 && j.FlagDelete == false);
        ...
        {
            var chk = await permission.HasPermission(1519, PermissionType.Add);
            if (!chk)
            {
                return Ok(await Result.AccessDenied("AccessDenied"));
            }
            try
            {
                ...
                var GetData = await hrServiceManager.HrProvisionService.GetEmployeeProvisionVacationData(filter);
                return Ok(GetData);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> Add(HrProvisionDto obj)
        {
            try
            {
                ...
                //  مخصص الاجازات
                obj.TypeId = 3;
                var add = await hrServiceManager.HrProvisionService.AddVacationProvision(obj);
                return Ok(add);
            }
            catch (Exception ex)
            {
                return Ok(await Result.FailAsync($"====== Exp in Add HRProvisionsVacationController, MESSAGE: {ex.Message}"));
            }
        }
        ...
        public async Task<ActionResult> Edit(HrProvisionEditDto obj)
        {
            ...
            {
                return Ok(await Result<HrProvisionEditDto>.FailAsync($"====== Exp in Edit HRProvisionsVacation, MESSAGE: {ex.Message}"));
            }
        }
        ...
        public async Task<ActionResult> CreateJournal(HrProvisionEntryAddDto obj)
        {
            try
            {
                ...
                obj.DocTypeId = 54;// نوع القيد قيد مخصص اجازة
                obj.Type = 3;// نوع القيد قيد مخصص اجازة
                var result = await hrServiceManager.HrProvisionService.CreateProvisionEntry(obj);
                return Ok(result);
            }
            catch (Exception ex)
            {
                return Ok(await Result<HrProvisionEditDto>.FailAsync($"====== Exp in CreateJournal HRProvisionsVacation, MESSAGE: {ex.Message}"));
            }
        }
