...
namespace Logix.MVC.LogixAPIs.HR
{
    ...
    public class HRAllowanceDeductionMController : BaseHrApiController
        {
            ...
            public HRAllowanceDeductionMController(IMainServiceManager mainServiceManager,
            ...
            public async Task<IActionResult> GetAll()
            {
                ...
                try
                {
                    var items = await hrServiceManager.HrAllowanceDeductionMService.GetAll(e => e.IsDeleted == false);
                    if (items.Succeeded)
                    {
                        var res = items.Data.AsQueryable();
                        res = res.OrderBy(e => e.Id);
                        return Ok(await Result<List<HrAllowanceDeductionMDto>>.SuccessAsync(res.ToList(), items.Status.message));
                    }
                    return Ok(items);
                }
                catch (Exception ex)
