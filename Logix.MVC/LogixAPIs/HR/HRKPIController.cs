...
{
    // تقييم الأداء
    public class HRKPIController : BaseHrApiController
    {
        ...
        public HRKPIController(IHrServiceManager hrServiceManager, IPermissionHelper permission, ILocalizationService localization, ICurrentData session, IMainServiceManager mainServiceManager)
        {
        ...
        {
            var chk = await permission.HasPermission(179, PermissionType.Show);
            if (!chk)
            {
        ...
        public async Task<IActionResult> Edit(long Id)
        {
            try
            {
                if (!await permission.HasPermission(179, PermissionType.Edit))
                {
                    return Ok(await Result.AccessDenied("AccessDenied"));
                }
            ...
            catch (Exception ex)
            {
                ...
                return Ok(await Result<HrDecisionsVw>.FailAsync($"Exception in HRKPIController Edit method: {ex.Message}"));
            }
        }
        ...
        public async Task<IActionResult> Delete(long id)
        {
            try
            {
                var chk = await permission.HasPermission(179, PermissionType.Delete);
                if (!chk)
                    return Ok(await Result.AccessDenied("AccessDenied"));
            ...
            {
                return Ok(await Result.FailAsync($"====== Exp in Delete HRKPIController, MESSAGE: {ex.Message}"));
            }
        }
        ...
        public async Task<IActionResult> KPIView(long Id)
        {
            ...
            {
                return Ok(await Result<HrLeaveVw>.FailAsync($"====== Exp in HRKPIController  KPIView, MESSAGE: {ex.Message}"));
            }
        }
        #endregion
        ...
        public async Task<ActionResult> Edit(HrKpiEditDto obj)
        {
            try
            {
                var chk = await permission.HasPermission(179, PermissionType.Edit);
                if (!chk)
                    return Ok(await Result.AccessDenied("AccessDenied"));
                ...
                if (string.IsNullOrEmpty(obj.Achievements)) return Ok(await Result<object>.FailAsync($"يجب ادخال  الانجازات خلال الفترة "));
                if (string.IsNullOrEmpty(obj.WeaknessesPoints)) return Ok(await Result<object>.FailAsync($"يجب ادخال نقاط الضعف  "));
                if (string.IsNullOrEmpty(obj.StrengthsPoints)) return Ok(await Result<object>.FailAsync($"يجب ادخال نقاط القوة  "));
                if (string.IsNullOrEmpty(obj.SuggestedTraining)) return Ok(await Result<object>.FailAsync($"يجب ادخال البرامج التدريبية المقترحة لتحسن الأداء  "));
                if (string.IsNullOrEmpty(obj.Recommendations)) return Ok(await Result<object>.FailAsync($"يجب ادخال التوصيات المقترحة  "));
                if (string.IsNullOrEmpty(obj.Note)) return Ok(await Result<object>.FailAsync($"يجب ادخال  ملاحظات أخرى"));
                if (obj.KpiTemId <= 0) return Ok(await Result<object>.FailAsync($"يجب ادخال  نموذج التقييم  "));

                obj.TypeId = 1;
                obj.ProbationResult = 0;
            ...
            {
                return Ok(await Result<HrKpiEditDto>.FailAsync($"====== Exp in Hr Hr HRKPIController   getById, MESSAGE: {ex.Message}"));
            }
        }
        ...
        public async Task<IActionResult> Approve(long id)
        {
            ...
            {
                return Ok(await Result.FailAsync($"====== Exp in Approve HRKPIController, MESSAGE: {ex.Message}"));
            }
        }
        ...
        public async Task<IActionResult> Reject(long id)
        {
            ...
            {
                return Ok(await Result.FailAsync($"====== Exp in Reject HRKPIController, MESSAGE: {ex.Message}"));
            }
        }
        #endregion
        ...
        public async Task<ActionResult> Add(HrKpiDto obj)
        {
            try
            {
                var chk = await permission.HasPermission(179, PermissionType.Add);
                if (!chk)
                    return Ok(await Result.AccessDenied("AccessDenied"));
                ...
                if (string.IsNullOrEmpty(obj.Recommendations)) return Ok(await Result<object>.FailAsync($"يجب ادخال التوصيات المقترحة  "));
                if (string.IsNullOrEmpty(obj.Note)) return Ok(await Result<object>.FailAsync($"يجب ادخال  ملاحظات أخرى"));
                if (obj.KpiTemId <= 0) return Ok(await Result<object>.FailAsync($"يجب اختيار  نموذج التقييم"));
                obj.TypeId = 1;
                obj.ProbationResult = 0;
            ...
            {
                return Ok(await Result.FailAsync($"====== Exp in Add  HRKPIController, MESSAGE: {ex.Message}"));
            }
        }
        ...
        public async Task<IActionResult> TemplateIDChanged(long Id)
        {
            ...
            {
                return Ok(await Result<HrDecisionsVw>.FailAsync($"====== Exp in Hr HRKPIController  TemplateIDChanged, MESSAGE: {ex.Message}"));
            }
        }
        ...
        public async Task<IActionResult> EmpCodeChanged(string EmpCode)
        {
            ...
            {
                return Ok(await Result<HrDecisionsVw>.FailAsync($"====== Exp in Hr HRKPIController  EmpCodeChanged, MESSAGE: {ex.Message}"));
            }
        }
