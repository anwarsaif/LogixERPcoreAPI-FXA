...
{
    public class HRClearanceTypeController : BaseHrApiController
        {
            ...
            public HRClearanceTypeController(IMainServiceManager mainServiceManager,
            ...
            public async Task<IActionResult> GetAll()
            {
                ...
                try
                {
                    var items = await hrServiceManager.HrClearanceTypeService.GetAll(e=> e.IsDeleted== false);
                    if (items.Succeeded)
                    {
                        var res = items.Data.AsQueryable();
                        res = res.OrderBy(e => e.TypeId);
                        return Ok(await Result<List<HrClearanceTypeDto>>.SuccessAsync(res.ToList(), items.Status.message));
                    }
                    return Ok(items);
                }
                catch (Exception ex)
