...
{
    public class HRArchiveFilesDetailController : BaseHrApiController
    {
        ...
        public HRArchiveFilesDetailController(IMainServiceManager mainServiceManager,
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            try
            {
                var items = await hrServiceManager.HrArchiveFilesDetailService.GetAll(e=>e.IsDeleted==false);
                if (items.Succeeded)
                {
                    var res = items.Data.AsQueryable();
                    res = res.OrderBy(e => e.Id);
                    return Ok(await Result<List<HrArchiveFilesDetailDto>>.SuccessAsync(res.ToList(), items.Status.message));
                }
                return Ok(items);
            }
            catch (Exception ex)
