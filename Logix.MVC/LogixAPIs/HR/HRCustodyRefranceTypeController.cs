...
{
    public class HRCustodyRefranceTypeController : BaseHrApiController
        {
            ...
            public HRCustodyRefranceTypeController(IMainServiceManager mainServiceManager,
            ...
            public async Task<IActionResult> GetAll()
            {
                ...
                try
                {
                    var items = await hrServiceManager.HrCustodyRefranceTypeService.GetAll();
                    if (items.Succeeded)
                    {
                        var res = items.Data.AsQueryable();
                        res = res.OrderBy(e => e.Id);
                        return Ok(await Result<List<HrCustodyRefranceTypeDto>>.SuccessAsync(res.ToList(), items.Status.message));
                    }
                    return Ok(items);
                }
                catch (Exception ex)
