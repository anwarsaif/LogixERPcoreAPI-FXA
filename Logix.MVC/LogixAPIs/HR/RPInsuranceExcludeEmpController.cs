...
{
    // تقرير بالمستبعدين من التأمين
    public class RPInsuranceExcludeEmpController : BaseHrApiController
    {
        ...
        public RPInsuranceExcludeEmpController(IPermissionHelper permission, ILocalizationService localization, ICurrentData session, IHrServiceManager hrServiceManager)
        {
        ...
        public async Task<IActionResult> Search(HrInsuranceEmpfilterRPDto filter)
        {
            ...
            filter.ClassId ??= 0;
            filter.BranchId ??= 0;
            var chk = await permission.HasPermission(1255, PermissionType.Show);
            if (!chk)
            {
