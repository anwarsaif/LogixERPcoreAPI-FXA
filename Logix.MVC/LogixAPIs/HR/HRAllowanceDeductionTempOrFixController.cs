...
public class HRAllowanceDeductionTempOrFixController : BaseHrApiController
{
    ...
    public HRAllowanceDeductionTempOrFixController(IMainServiceManager mainServiceManager,
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        try
        {
            var items = await hrServiceManager.HrAllowanceDeductionTempOrFixService.GetAll();
            if (items.Succeeded)
            {
                var res = items.Data.AsQueryable();
                res = res.OrderBy(e => e.Id);
                return Ok(await Result<List<HrAllowanceDeductionTempOrFixDto>>.SuccessAsync(res.ToList(), items.Status.message));
            }
            return Ok(items);
        }
        catch (Exception ex)
