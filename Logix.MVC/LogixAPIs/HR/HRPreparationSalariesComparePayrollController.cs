...
{
    //   مقارنة المدخلات
    public class HRPreparationSalariesComparePayrollController : BaseHrApiController
    {
        ...
        public HRPreparationSalariesComparePayrollController(IHrServiceManager hrServiceManager, IPermissionHelper permission, ICurrentData session, ILocalizationService localization, IMainServiceManager mainServiceManager)
        {
        ...
        public async Task<IActionResult> BINDGRID1(HrPayrollCompareFilterDto filter)
        {
            ...
            try
            {
                ...
                var getData = await hrServiceManager.HrPreparationSalaryService.PreparationSalariesPayrollCompare(filter, 1);
        ...
        public async Task<IActionResult> BINDGRID2(HrPayrollCompareFilterDto filter)
        {
            ...
            try
            {
                ...
                var getData = await hrServiceManager.HrPreparationSalaryService.PreparationSalariesPayrollCompare(filter, 2);
                if (getData.Data.Count() > 0)
                {
                    return Ok(await Result<object>.SuccessAsync(getData.Data.ToList()));
                }
        ...
        public async Task<IActionResult> BINDGRID3(HrPayrollCompareFilterDto filter)
        {
            ...
            try
            {
                ...
                var getData = await hrServiceManager.HrPreparationSalaryService.PreparationSalariesPayrollCompare(filter, 3);
                if (getData.Data.Count() > 0)
                {
                    return Ok(await Result<object>.SuccessAsync(getData.Data.ToList()));
                }
        ...
        public async Task<IActionResult> BINDGRID4(HrPayrollCompareFilterDto filter)
        {
            ...
            try
            {
                ...
                var getData = await hrServiceManager.HrPreparationSalaryService.PreparationSalariesPayrollCompare(filter, 4);
                if (getData.Data.Count() > 0)
                {
                    return Ok(await Result<object>.SuccessAsync(getData.Data.ToList()));
                }
