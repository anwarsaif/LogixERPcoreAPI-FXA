...
{
    public class HrAttendanceBioTimeController : BaseHrApiController
        {
            ...
            public HrAttendanceBioTimeController(IMainServiceManager mainServiceManager,
            ...
            public async Task<IActionResult> GetAll()
            {
                ...
                try
                {
                    var items = await hrServiceManager.HrAttendanceBioTimeService.GetAll();
                    if (items.Succeeded)
                    {
                        var res = items.Data.AsQueryable();
                        res = res.OrderBy(e => e.Id);
                        return Ok(await Result<List<HrAttendanceBioTimeDto>>.SuccessAsync(res.ToList(), items.Status.message));
                    }
                    return Ok(items);
                }
                catch (Exception ex)
