...
{
    public class WfStepController : BaseWfController
    {
        private readonly IWFServiceManager wfServiceManager;
        private readonly IPermissionHelper permission;
        private readonly ILocalizationService localization;

        public WfStepController(IWFServiceManager wfServiceManager,
        ...
        [HttpPost("Add")]
        public async Task<ActionResult> Add(WfStepDto obj)
        {
            try
            {
                ...
                var add = await wfServiceManager.WfStepService.Add(obj);
                return Ok(add);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetByIdForEdit(int id)
        {
            try
            {
                ...
                var item = await wfServiceManager.WfStepService.GetForUpdate<WfStepEditDto>(id);
                return Ok(item);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Edit")]
        public async Task<ActionResult> Edit(WfStepEditDto obj)
        {
            try
            {
                var chk = await permission.HasPermission(267, PermissionType.Edit);
                ...
                obj.LevelNo ??= 0; obj.CommitteeType ??= 0; obj.CommitteeId ??= 0; obj.DecisionsType ??= 0;
                var update = await wfServiceManager.WfStepService.Update(obj);
                return Ok(update);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                var delete = await wfServiceManager.WfStepService.Remove(id);
                return Ok(delete);
            }
            catch (Exception ex)
            {
