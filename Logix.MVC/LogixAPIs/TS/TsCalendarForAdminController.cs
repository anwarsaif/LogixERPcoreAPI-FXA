...
{
    public class TsCalendarForAdminController : BaseTsApiController
    {
        private readonly ITsServiceManager tsServiceManager;
        private readonly ICurrentData session;
        private readonly IPermissionHelper permission;

        public TsCalendarForAdminController(ITsServiceManager tsServiceManager,ICurrentData session,
        ...
        public async Task<IActionResult> RetrieveUsersForAdmin()
        {
            try
            {
                var chk = await permission.HasPermission(255, PermissionType.Show);
                chk = await permission.HasPermission(456, PermissionType.Show);
                if (!chk)
                    return Ok(await Result.AccessDenied("AccessDenied"));
                var items = await tsServiceManager.TsAppointmentService.GetAll();

                return Ok(await Result<IEnumerable<TsAppointmentDto>>.SuccessAsync(items.Data, $"Search Completed {items.Data.Count()}", 200));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> SaveEvent(SaveEventDto entity)
        {
            try
            {
                var chk = await permission.HasPermission(255, PermissionType.Add);
                chk = await permission.HasPermission(456, PermissionType.Add);
                if (!chk)
                    return Ok(await Result.AccessDenied("AccessDenied"));
        ...
        public async Task<IActionResult> UpdateDragEvents(long id, string startDate, bool allDay)
        {
            try
            {
                var chk = await permission.HasPermission(255, PermissionType.Edit);
                chk = await permission.HasPermission(456, PermissionType.Edit);
                if (!chk)
                    return Ok(await Result.AccessDenied("AccessDenied"));
