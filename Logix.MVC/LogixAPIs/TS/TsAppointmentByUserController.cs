...
{
    public class TsAppointmentByUserController : BaseTsApiController
    {
        ...
        public TsAppointmentByUserController(
        ...
        public async Task<IActionResult> GetAll()
        {
            try
            {
                ...
                var items = await tsServiceManager.TsAppointmentService.GetAll(x => x.IsDeleted == false && x.UserId == session.UserId);
        ...
        [HttpPost("Search")]
        public async Task<IActionResult> Search(PaginatedRequest<TsAppointmentByUserFilterDto> request)
        {
            try
            {
                var chk = await permission.HasPermission(1092, PermissionType.Show);
