...
{
    public class PmCertificateCompletionController : BasePMApiController
    {
        ...
        public PmCertificateCompletionController(IPMServiceManager pmServiceManager,
        ...
        public async Task<IActionResult> Search(PmExtractTransactionFilterDto filter)
        {
            try
            {
                var chk = await permission.HasPermission(1843, PermissionType.Show);
                if (!chk)
                    return Ok(await Result.AccessDenied("AccessDenied"));

                filter.PaymentTermsId =5;
        ...
        public async Task<IActionResult> Add(PmExtractTransactionAddDto entity)
        {
            ...
            var chk = await permission.HasPermission(1843, PermissionType.Add);
            ...
            try
            {
                ...
                var items = await pmServiceManager.PmCertificateCompletionService.Add(entity);
...
/*
                if (id <= 0)
                    return Ok(await Result.FailAsync($"{localization.GetMessagesResource("NoIdInDelete")}"));
*/
                var result = await pmServiceManager.PmCertificateCompletionService.GetProjectExtractInfo(Code);
                return Ok(result);  
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetByIdForEdit(long Id)
        {
            ...
            try
            {
                ...
                var items = await pmServiceManager.PmCertificateCompletionService.GetForEditById(Id);
        ...
        public async Task<IActionResult> CreateACC(PmExtractTransactionACCCreateDto entity)
        {
            ...
            try
            {
                ...
                var items = await pmServiceManager.PmCertificateCompletionService.CreateACC(entity);
